const AssetCategoryPage = {
  ...
  async loadCategories() {
    ...
    try {
      const res = await fetch(`${BASE_URL}/asset-category/get.php?${params}`, {
      ...
      if (data.success) {
        ...
        Toast.error(data.message || 'Gagal memuat data kategori');
      }
    } catch (e) {
  ...
  renderTable(categories, pagination) {
    ...
    if (!categories || !categories.length) {
      tbody.innerHTML = '<tr><td colspan="4" class="table-empty">Belum ada data kategori</td></tr>';
      ...
      <tr>
        ...
        <td>
          <div class="action-group">
            <button class="btn btn-icon btn-edit btn-sm" onclick="AssetCategoryPage.openEdit(${c.id})" title="Edit">
              <i class="fa-solid fa-pen"></i>
            </button>
            <button class="btn btn-icon btn-delete btn-sm" onclick="AssetCategoryPage.openDelete(${c.id}, '${this.escapeHtml(c.name)}')" title="Hapus">
  ...
  renderPagination(p) {
    ...
    let html = `
      <button class="page-btn ${p.current_page === 1 ? 'disabled' : ''}"
        onclick="AssetCategoryPage.goPage(${p.current_page - 1})">
        <i class="fa-solid fa-chevron-left"></i>
      </button>`;

    for (let i = 1; i <= p.last_page; i++) {
      html += `<button class="page-btn ${i === p.current_page ? 'active' : ''}"
        onclick="AssetCategoryPage.goPage(${i})">${i}</button>`;
    }
  ...
  openForm() {
    this.editId = null;
    document.getElementById('modalFormTitle').textContent = 'Tambah Kategori';
  ...
  async openEdit(id) {
    this.editId = id;
    document.getElementById('modalFormTitle').textContent = 'Edit Kategori';
    
    try {
      const res = await fetch(`${BASE_URL}/asset-category/get.php?id=${id}`, {
        headers: { 'Authorization': `Bearer ${Storage.getToken()}` }
      });
      const data = await res.json();
  ...
  async saveCategory() {
    ...
    if (!name) {
      ...
      document.getElementById('nameError').textContent = 'Nama kategori wajib diisi';
      return;
    }
    ...
    const url = isEdit ? `${BASE_URL}/asset-category/update.php` : `${BASE_URL}/asset-category/create.php`;
  ...
  async confirmDelete() {
    ...
    try {
      const res = await fetch(`${BASE_URL}/asset-category/delete.php`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${Storage.getToken()}`
