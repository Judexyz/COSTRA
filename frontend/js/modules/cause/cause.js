const CausePage = {
  ...
  async loadCategories() {
    ...
    try {
      const res = await fetch(`${BASE_URL}/cause/get.php?${params}`, {
      ...
      if (data.success) {
        ...
        Toast.error(data.message || 'Gagal memuat data akar masalah');
      }
    } catch (e) {
  ...
  renderTable(categories, pagination) {
    ...
    if (!categories || !categories.length) {
      tbody.innerHTML = '<tr><td colspan="4" class="table-empty">Belum ada data akar masalah</td></tr>';
      ...
      <tr>
        ...
        <td>
          <div class="action-group">
            <button class="btn btn-icon btn-edit btn-sm" onclick="CausePage.openEdit(${c.id})" title="Edit">
              <i class="fa-solid fa-pen"></i>
            </button>
            <button class="btn btn-icon btn-delete btn-sm" onclick="CausePage.openDelete(${c.id}, '${this.escapeHtml(c.name)}')" title="Hapus">
  ...
  renderPagination(p) {
    ...
    let html = `
      <button class="page-btn ${p.current_page === 1 ? 'disabled' : ''}"
        onclick="CausePage.goPage(${p.current_page - 1})">
        <i class="fa-solid fa-chevron-left"></i>
      </button>`;

    for (let i = 1; i <= p.last_page; i++) {
      html += `<button class="page-btn ${i === p.current_page ? 'active' : ''}"
        onclick="CausePage.goPage(${i})">${i}</button>`;
    }
  ...
  openForm() {
    this.editId = null;
    document.getElementById('modalFormTitle').textContent = 'Tambah Akar Masalah';
  ...
  async openEdit(id) {
    this.editId = id;
    document.getElementById('modalFormTitle').textContent = 'Edit Akar Masalah';
    
    try {
      const res = await fetch(`${BASE_URL}/cause/get.php?id=${id}`, {
        headers: { 'Authorization': `Bearer ${Storage.getToken()}` }
      });
      const data = await res.json();
      
      if (!data.success || !data.data || !data.data[0]) {
        Toast.show(data.message || 'Data akar masalah tidak ditemukan', 'error');
        return;
      }
  ...
  async saveCategory() {
    ...
    if (!name) {
      ...
      document.getElementById('nameError').textContent = 'Nama akar masalah wajib diisi';
      return;
    }
    ...
    const url = isEdit ? `${BASE_URL}/cause/update.php` : `${BASE_URL}/cause/create.php`;
  ...
  async confirmDelete() {
    ...
    try {
      const res = await fetch(`${BASE_URL}/cause/delete.php`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${Storage.getToken()}`
