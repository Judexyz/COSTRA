const OvertimeHistory = {
  ...
  renderTable() {
    ...
    if (filterStatusVal) {
      filteredData = filteredData.filter(item => item.status === filterStatusVal);
    }
    ...
    filteredData.forEach(item => {
      ...
      let actionHtml = `<span style="${statusStyle}">${item.status}</span>`;
  ...
  bindDownload() {
    ...
    btnDownload.addEventListener('click', () => {
      if (this.requests.length === 0) {
        if (typeof Toast !== 'undefined') Toast.show('No history to download', 'error');
        return;
      }
