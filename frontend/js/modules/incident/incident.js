const IncidentPage = {
  ...
  bindEvents() {
    ...
    document.getElementById('searchInput').addEventListener('input', () => { this.currentPage = 1; this.loadIncidents(); });
    document.getElementById('filterStatus').addEventListener('change', () => { this.currentPage = 1; this.loadIncidents(); });
    document.getElementById('filterPriority').addEventListener('change', () => { this.currentPage = 1; this.loadIncidents(); });
    document.getElementById('btnRefresh').addEventListener('click', () => this.loadIncidents());
    document.getElementById('btnExportPDF').addEventListener('click', () => this.exportPDF());
    
    document.getElementById('btnSaveForm').addEventListener('click', () => this.saveIncident());
  ...
  async loadIncidents() {
    ...
    try {
      const res = await fetch(`${BASE_URL}/incident/get.php?${params}`, {
  ...
  renderTable(tickets, pagination) {
    ...
    tbody.innerHTML = tickets.map((t, i) => `
      <tr>
        <td><strong>${t.incident_no}</strong></td>
        ...
        <td>
          <div class="action-group">
            ${(t.status !== 'closed' && t.status !== 'solved') ? `<button class="btn btn-icon btn-success btn-sm" onclick="IncidentPage.openSolveModal(${t.id})" title="Selesaikan"><i class="fa-solid fa-check-double"></i></button>` : ''}
            ${t.status !== 'rejected' ? `<button class="btn btn-icon btn-reject btn-sm" onclick="IncidentPage.quickStatus(${t.id}, 'rejected')" title="Tolak"><i class="fa-solid fa-xmark"></i></button>` : ''}
            <button class="btn btn-icon btn-edit btn-sm" onclick="IncidentPage.openEdit(${t.id})" title="Edit">
              <i class="fa-solid fa-pen"></i>
            </button>
            <button class="btn btn-icon btn-delete btn-sm" onclick="IncidentPage.openDelete(${t.id}, '${t.incident_no}')" title="Hapus">
  ...
  renderPagination(p) {
    ...
    let html = `
      <button class="page-btn ${p.current_page === 1 ? 'disabled' : ''}"
        onclick="IncidentPage.goPage(${p.current_page - 1})">
        <i class="fa-solid fa-chevron-left"></i>
      </button>`;

    for (let i = 1; i <= p.last_page; i++) {
      html += `<button class="page-btn ${i === p.current_page ? 'active' : ''}"
        onclick="IncidentPage.goPage(${i})">${i}</button>`;
    }
  ...
  async openEdit(id) {
    this.editId = id;
    document.getElementById('modalFormTitle').textContent = 'Edit Insiden';
    
    await this.loadLookups();

    try {
      const res = await fetch(`${BASE_URL}/incident/get.php?search=`, {
        headers: { 'Authorization': `Bearer ${Storage.getToken()}` }
      });
      const data = await res.json();
  ...
  async saveIncident() {
    ...
    const url = isEdit ? `${BASE_URL}/incident/update.php` : `${BASE_URL}/incident/create.php`;
  ...
  async quickStatus(id, status) {
    try {
      const res = await fetch(`${BASE_URL}/incident/update_status.php`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${Storage.getToken()}`
  ...
  async confirmDelete() {
    ...
    try {
      const res = await fetch(`${BASE_URL}/incident/delete.php`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${Storage.getToken()}`
  ...
  async exportPDF() {
    try {
      ...
      doc.setFontSize(18);
      doc.text('Laporan Data Insiden - Himawari Digi', 40, 40);
      doc.setFontSize(11);
