...
if ($status === 'solved') {
    $stmt = $db->prepare('UPDATE incidents SET status = ?, cause_id = ?, impact_id = ?, solution_note = ?, updated_at = NOW() WHERE id = ? AND deleted_at IS NULL');
    $stmt->bind_param('siisi', $status, $cause_id, $impact_id, $solution_note, $id);
} else {
    $stmt = $db->prepare('UPDATE incidents SET status = ?, updated_at = NOW() WHERE id = ? AND deleted_at IS NULL');
    $stmt->bind_param('si', $status, $id);
}

if ($stmt->execute()) {
    echo json_encode(['success' => true, 'message' => 'Status tiket berhasil diperbarui']);
    logAudit($db, $user['id'], 'UPDATE', 'INCIDENT', "Mengubah status insiden ID {$id} menjadi {$status}");
} else {
