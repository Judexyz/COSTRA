...
$stmt = $db->prepare('UPDATE service_requests SET
    asset_id = ?, client_id = ?, user_id = ?,
    priority = ?, severity = ?, status = ?,
    description = ?, cause_id = ?, impact_id = ?, updated_at = NOW()
    WHERE id = ?');

$stmt->bind_param('iiissssiii',
    $asset_id, $client_id, $user_id,
    $priority, $severity, $status,
    $description, $cause_id, $impact_id, $id
);

if ($stmt->execute()) {
    echo json_encode(['success' => true, 'message' => 'Ticket berhasil diperbarui']);
    logAudit($db, $user['id'], 'UPDATE', 'SERVICE REQUEST', "Memperbarui request dengan ID: {$id}");
} else {
