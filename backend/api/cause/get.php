...
if ($id) {
    $stmt = $db->prepare('SELECT * FROM causes WHERE id = ? AND deleted_at IS NULL');
...
$count_sql  = "SELECT COUNT(*) as total FROM causes WHERE $where";
...
$sql  = "SELECT id, name, description, created_at FROM causes WHERE $where ORDER BY $sort $order LIMIT ? OFFSET ?";
$stmt = $db->prepare($sql);
